...
import FuzzySet from "fuzzyset"; // Import the fuzzyset library
...
export class DedupeRecords {
  ...
  private async processRecords(allRecords: Flatfile.RecordWithLinks[]) {
    ...
    // Populate the fuzzy set with existing uniqueIds
    for (const uniqueId of this.uniqueRecordsByID.keys()) {
      fuzzySet.add(uniqueId);
    }

    for (const record of allRecords) {
      const uniqueId = record.values[this.mergeKey]?.value?.toString();
      const potentialMatches = fuzzySet.get(uniqueId, null, 0.6); // set a threshold

      if (potentialMatches && potentialMatches.length) {
        // Get the best match based on the highest score
        const [highestScore, bestMatch] = potentialMatches.sort(
          (a, b) => b[0] - a[0],
        )[0];
        await this.handleDuplicateRecord(bestMatch, record);
      } else {
        this.handleUniqueRecord(uniqueId, record);
        fuzzySet.add(uniqueId); // Add to fuzzy set for future comparisons
      }
    }
  }
  ...
  private async handleDuplicateRecord(
    ...
    // Use bestMatch instead of uniqueId
    const existingRecord = this.uniqueRecordsByID.get(bestMatch);
    ...
    this.uniqueRecordsByID.set(bestMatch, {
      ...existingRecord,
      id: record.id,
